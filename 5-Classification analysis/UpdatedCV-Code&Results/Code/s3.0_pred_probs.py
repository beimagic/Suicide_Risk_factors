...
for region_fold in tqdm(region_fold_lst):
    ...
    '''
    Number of features selected was determined based on sequential forward selection procedure in previous analysis
    '''
    nb_f = get_nb_f(auc_imp_df)
    ...
    Saving models
    '''
    with open(dpath + 'Results/Suicide/TestingFold_'+str(region_fold)+'/s3.0_top'+str(nb_f)+'_features_trained_model.pkl', 'wb') as f:
    ...
    pred_df.to_csv(dpath + 'Results/Suicide/TestingFold_'+str(region_fold)+'/s3.0_top'+str(nb_f)+'_features_predicted_risks.csv', index = False)
