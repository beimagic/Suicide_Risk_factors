...
for region_fold in tqdm(region_fold_lst):
    ...
    For regional cross validation, merely using the training data (nine regional fold of data) to perform the sequential forward selection procedure
    The sequential forward selection was implemented based on the s1_Importance.csv within the training regional fold calculated in s1_ImpRanking.py
    '''
    ...
    for f in tqdm(imp_f_lst):
        '''
        Adding each feature sequentially
        '''
        tmp_f.append(f)
        my_X = mydf[tmp_f]
        in_cv_AUC_lst = []
        ...
        for inner_cv_fold_id in inner_cv_fold_lst:
            '''
            Within each inner fold, trained lightGBM model, get predicted risks and evaluated the statistical improvement based on DeLong Test
            '''
    ...
    '''
    Save the sequential AUCs and p-values from DeLong tests
    '''
